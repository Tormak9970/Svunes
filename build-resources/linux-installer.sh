...
uninstallComment="Uninstalls all files related to Tunistic."

currentVersion="v0.0.0"

# Show starting message.
echo -e "${infoCyan}[INFO]${noColor}: Installing Tunistic $currentVersion..."
echo ""
...
# Check if the .tunistic directory already exists.
if [ -d "$HOME/.tunistic" ]; then
...
rm -rf "$HOME"/.tunistic/tunistic.AppImage 2>/dev/null
echo -e "${infoCyan}[INFO]${noColor}: Downloading AppImage..."
curl -L https://github.com/Tormak9970/Tunistic/releases/download/$currentVersion/tunistic.AppImage --output ~/.tunistic/tunistic.AppImage 
chmod 700 ~/.tunistic/tunistic.AppImage
echo -e "${successGreen}[INFO]${noColor}: Downloaded AppImage."
echo ""

# Download the uninstall script from GitHub.
rm -rf "$HOME"/.tunistic/linux-uninstaller.sh 2>/dev/null
echo -e "${infoCyan}[INFO]${noColor}: Downloading Uninstall Script..."
curl -L https://raw.githubusercontent.com/Tormak9970/Tunistic/$currentVersion/build-resources/linux-uninstaller.sh --output ~/.tunistic/linux-uninstaller.sh
chmod 700 ~/.tunistic/linux-uninstaller.sh
echo -e "${successGreen}[INFO]${noColor}: Downloaded Uninstall Script."
echo ""

# Download the icon from GitHub.
rm -rf "$HOME"/.tunistic/tunistic.png 2>/dev/null
echo -e "${infoCyan}[INFO]${noColor}: Downloading Icon..."
curl https://raw.githubusercontent.com/Tormak9970/Tunistic/$currentVersion/public/logo.png --output ~/.tunistic/tunistic.png
echo -e "${successGreen}[INFO]${noColor}: Downloaded Icon."
echo ""


absSarmPath="$HOME/.tunistic"
appImagePath="$absSarmPath/tunistic.AppImage"
iconPath="$absSarmPath/tunistic.png"

cd $absSarmPath
xdg-icon-resource install tunistic.png --size 64
xdg-icon-resource install tunistic.png --size 128
xdg-icon-resource install tunistic.png --size 256
...
rm -rf "$HOME"/.local/share/applications/tunistic.desktop 2>/dev/null
echo -e "$shortcutContents" >> $HOME/.local/share/applications/tunistic.desktop
chmod 700 $HOME/.local/share/applications/tunistic.desktop
echo -e "${successGreen}[INFO]${noColor}: Made Start Menu Shortcut."

# Create the desktop launcher.
rm -rf "$HOME"/Desktop/tunistic.desktop 2>/dev/null
echo -e "$shortcutContents" >> $HOME/Desktop/tunistic.desktop
chmod 700 $HOME/Desktop/tunistic.desktop
echo -e "${successGreen}[INFO]${noColor}: Made Desktop Shortcut."

# Create the start menu uninstaller.
rm -rf "$HOME"/.local/share/applications/uninstall-tunistic.desktop 2>/dev/null
echo -e "$uninstallSarmContent" >> $HOME/.local/share/applications/uninstall-tunistic.desktop
chmod 700 $HOME/.local/share/applications/uninstall-tunistic.desktop
echo -e "${successGreen}[INFO]${noColor}: Made Start Menu Uninstall Shortcut."

# Create the desktop uninstaller.
rm -rf "$HOME"/Desktop/uninstall-tunistic.desktop 2>/dev/null
echo -e "$uninstallSarmContent" >> $HOME/Desktop/uninstall-tunistic.desktop
chmod 700 $HOME/Desktop/uninstall-tunistic.desktop
...
echo -e "${successGreen}[INFO]${noColor}: Instalation of Tunistic $currentVersion complete."
echo ""
echo -e "${tipBlue}[TIP]${noColor}: Feel free to delete this script. Tunistic will notify you when new updates are available!"