...
<div data-tauri-drag-region on:contextmenu|preventDefault class="titlebar">
  <div class="info" style="width: 131px;">
    <img src="/logo.svg" alt="logo" height="20" style="margin-left: 7px; margin-right: 14px;" />
    <slot />
  </div>
  <div class="info">
    <!-- svelte-ignore missing-declaration -->
    <div style="margin-left: 8px; margin-right: 30px; width: 100px">{title + " - v" + APP_VERSION}</div>
  </div>
  <div class="btns">
    <div bind:this={minimize} class="titlebar-button" id="titlebar-minimize">
      <svg width="10" height="2" viewBox="0 0 11 2">
        <path d="m11 0v1h-11v-1z" stroke-width="0.25" style="fill: var(--md-sys-color-on-secondary-container);" />
      </svg>
    </div>
    ...
    <div bind:this={close} class="titlebar-button" id="titlebar-close">
      <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
        <path fill="none" stroke="var(--md-sys-color-on-secondary-container)" stroke-width="2" d="M3,3 L21,21 M3,21 L21,3" />
      </svg>
    </div>
  </div>
</div>
