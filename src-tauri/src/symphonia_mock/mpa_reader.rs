...
use symphonia::core::formats::{Cue, FormatOptions, Packet, Track};
use symphonia::core::io::MediaSourceStream;
use symphonia::core::units::TimeBase;
use symphonia::core::support_format;

use symphonia::core::checksum::Crc16AnsiLe;
use symphonia::core::codecs::CodecParameters;
use symphonia::core::errors::{seek_error, Result, SeekErrorKind};
use symphonia::core::formats::prelude::*;
...
impl FormatReader for MpaReader {
  fn try_new(mut source: MediaSourceStream, options: &FormatOptions) -> Result<Self> {
    ...
    params
      ...
      .with_bits_per_sample(header.bitrate)
  ...
  fn next_packet(&mut self) -> Result<Packet> {
    ...
    if self.options.enable_gapless {
      symphonia::core::formats::util::trim_packet(
        &mut packet,
...
impl MpaReader {
  ...
  fn preseek_coarse(&mut self, required_ts: u64, delay: u64) -> Result<()> {
    ...
    let packet_pos = ((u128::from(required_ts) * u128::from(audio_byte_len)) / u128::from(duration)) as u64;
    ...
    let ts = ((u128::from(seeked_pos - self.first_packet_pos) * u128::from(duration)) / u128::from(audio_byte_len)) as u64;
